...
BS = chr(92)
NLTOK = BS + 'n'   # literal backslash + n
...
for a, b in zip(src['items'], out['items']):
    ...
    if a['t'].count(NLTOK) != b['t'].count(NLTOK): bad.append(('nl', a['i'], a['t'].count(NLTOK), b['t'].count(NLTOK)))
    if NL in b['t']: bad.append(('realNL', a['i']))
