"""Decode the NON-differing res_lang textures + res_lang_exp.bin textures
to find the radial command menu images (kanji identical across CJK)."""
...
PNG_DIR = os.path.join(OUT, 'png_rest')
    ...
    for i, (off, size) in enumerate(parse_link(d)):
        ...
        if blob[:4] == b'LINK':
            for j, (o2, s2) in enumerate(parse_link(blob)):
                subs[(i, j)] = kt_unwrap(blob[o2:o2 + s2])
        else:
            subs[(i, -1)] = kt_unwrap(blob)
    return subs

thumbs = []

# 1) non-differing textures in res_lang.bin
jp = load_subs(os.path.join(SRC, r'romfs\RES_JP\res_lang.bin'))
sc = load_subs(os.path.join(SRC, r'romfs\RES_SC\res_lang.bin'))
n_same = 0
for k in sorted(jp.keys()):
    v = jp[k]
    if v[:4] != b'GT1G':
        continue
    if k in sc and hashlib.sha1(v).digest() == hashlib.sha1(sc[k]).digest():
        n_same += 1
        for t, (img, info) in enumerate(decode_g1t(v)):
            name = f'L{k[0]:02d}_{k[1]:03d}_{t}'
            if img:
                img.save(os.path.join(PNG_DIR, name + '.png'))
                thumbs.append((name, info, img))
print(f'{n_same} same-as-SC G1T decoded')

# 2) res_lang_exp.bin (all)
exp = load_subs(os.path.join(SRC, r'romfs\RES_JP\res_lang_exp.bin'))
n_exp = 0
for k in sorted(exp.keys()):
    ...
    n_exp += 1
    for t, (img, info) in enumerate(decode_g1t(v)):
        name = f'E{k[0]:02d}_{k[1]:03d}_{t}'
        if img:
            img.save(os.path.join(PNG_DIR, name + '.png'))
            thumbs.append((name, info, img))
print(f'{n_exp} exp G1T decoded, total thumbs {len(thumbs)}')
...
for s in range(0, len(thumbs), per):
    ...
    sheet.save(os.path.join(OUT, 'contact_sheets', f'rest_{sheets:02d}.jpg'), quality=82)
    sheets += 1
print(f'{sheets} sheets written')
