"""Find and decode G1T textures inside RES/res_else.bin (shared UI resources)."""
...
PNG_DIR = os.path.join(OUT, 'png_else')
...
with open(os.path.join(SRC, r'romfs\RES\res_else.bin'), 'rb') as f:
    d = f.read()
toc = parse_link(d)
thumbs = []
n_g1t = 0
for i, (off, size) in enumerate(toc):
    ...
    if dec[:4] != b'GT1G':
        continue
    n_g1t += 1
    ntex = struct.unpack_from('<I', dec, 0x10)[0]
    for t, (img, info) in enumerate(decode_g1t(dec)):
        if img is None or img.width < 16 or img.height < 16:
            continue
        name = f'X{i:03d}_{t:03d}'
        img.save(os.path.join(PNG_DIR, name + '.png'))
        thumbs.append((name, info, img))
print(f'{n_g1t} G1T entries, {len(thumbs)} textures >=16px decoded')
...
for s in range(0, len(thumbs), per):
    ...
    sheet.save(os.path.join(OUT, 'contact_sheets', f'else_{sheets:02d}.jpg'), quality=82)
    sheets += 1
