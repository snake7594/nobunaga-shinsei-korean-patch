...
src_raw = io.open(base + r"\batches\b0015_dialog.json", encoding="utf-8").read()
...
out = json.load(io.open(base + r"\translated\b0015_dialog.json", encoding="utf-8"))
...
with io.open(base + r"\translated\b0015_dialog.json", "w", encoding="utf-8") as f:
    json.dump({"items": items}, f, ensure_ascii=False, indent=0)

chk = io.open(base + r"\translated\b0015_dialog.json", encoding="utf-8").read()
